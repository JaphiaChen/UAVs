...
def solve_info(in_stream):
    uavs = {}
    no_fly_zones = {}

    # 此行是UAV信息项介绍
    _ = in_stream.readline().strip()
    N = int(in_stream.readline())       # UAV总数
    for _ in range(N):      # 循环从txt中读取信息，生成UAV类的多个实例
        items = in_stream.readline().strip().split(',')     # 每行先读到items变量
        id = items[0]   # 首位是id
    ...
    for id in uavs:
        targets = {}
        M = int(in_stream.readline())       # Target总数
    ...
    # 此行是禁飞区信息项介绍
    _ = in_stream.readline().strip()
    L = int(in_stream.readline())       # 禁飞区总数
    for _ in range(L):      # 循环从txt中读取信息，生成Target类的多个实例
        item1 = in_stream.readline().strip().split(',')     # 每行先读到items变量
        item2 = in_stream.readline().strip().split(',')
        id = item1[0]
